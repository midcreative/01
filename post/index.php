...
if (!$post) {
    ...
    echo '<h1>404 ???æ‰??∞Ê≠§ÁØáÊó•Ë®?/h1><a href="/">ËøîÂ?È¶ñÈ?</a>';
    exit;
}
...
<nav class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-[#E0F2ED]">
    <div class="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
        ...
        <span class="text-sm text-slate-500 font-medium truncate max-w-xs"><?= htmlspecialchars($post['category_name'] ?? '?™Â?È°?) ?></span>
    </div>
</nav>
...
<main class="max-w-4xl mx-auto px-4 py-12 md:py-20">
    ...
    <header class="mb-10">
        <div class="flex flex-wrap items-center gap-2 mb-4 text-xs font-bold">
            <span class="px-3 py-1 rounded-full <?= $badgeClass ?>"><?= htmlspecialchars($post['category_name'] ?? '?™Â?È°?) ?></span>
            <span class="text-slate-400 flex items-center gap-1">
                <i data-lucide="map-pin" class="w-3 h-3 brand-green"></i> <?= htmlspecialchars($post['town_name'] ?? '?™Ê?ÂÆ?) ?>
            </span>
    ...
    <div class="mt-12 pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-brand-green rounded-xl flex items-center justify-center text-white font-serif font-black">ÊΩ?/div>
            <div>
                <div class="font-bold text-slate-800 text-sm">ÊΩòÁÇ©Á¶?/div>
                <div class="text-xs text-slate-400">Â±èÊù±Á∏?¨¨‰∏âÈÅ∏?Ä?çÂ?Ëæ¶ÂÖ¨ÂÆ?/div>
            </div>
        </div>
        <a href="/" class="text-sm brand-green font-black flex items-center gap-1 hover:gap-3 transition-all">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> ËøîÂ??çÂ??•Ë??óË°®
        </a>
    </div>
</main>
...
FrontLayout::render($content, [
    'title'       => htmlspecialchars($post['title']) . ' | ÊΩòÁÇ©Á¶??çÂ??•Ë?',
