...
try {
    ...
    $stmtTown->execute(['?°åŸ¤??, 6]);
    $stmtTown->execute(['ç«¹ç”°??, 7]);
    echo "<p>[OK] Super-added '?°åŸ¤?? and 'ç«¹ç”°?? into 'post_towns' table.</p>";

    // 2. Extend `petitions` table ENUM
    $sql2 = "ALTER TABLE petitions MODIFY COLUMN town ENUM('?¨éƒ¨?°å?','æ½®å???,'?°åŸ¤??,'?§å???,'?¬å???,'ç«¹ç”°??,'?‹å¯®??);";
    $pdo->exec($sql2);
    echo "<p>[OK] Updated 'petitions' table ENUM.</p>";

    // Extend `volunteer_jobs` table
    $sql3 = "ALTER TABLE volunteer_jobs MODIFY COLUMN town ENUM('?¨éƒ¨?°å?','æ½®å???,'?°åŸ¤??,'?§å???,'?¬å???,'ç«¹ç”°??,'?‹å¯®??);";
    $pdo->exec($sql3);
