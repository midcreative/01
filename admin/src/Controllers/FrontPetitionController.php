...
final class FrontPetitionController
{
    ...
    /**
     * Start the LINE login flow for proposing a new petition
     */
    public function redirectForProposeLogin(string $title, string $description, string $category, string $town): void
    {
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }

        // Store propose data in session
        $_SESSION['propose_data'] = [
            'title' => $title,
        ...
        // Generate and store state for CSRF protection
        $state = bin2hex(random_bytes(16));
        $_SESSION['propose_login_state'] = $state;
    ...
    /**
     * Handle the callback from LINE for proposing a petition
     */
    public function handleProposeCallback(array $params): void
    {
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
        ...
        $code  = $params['code'] ?? null;
        $state = $params['state'] ?? null;
        $proposeData = $_SESSION['propose_data'] ?? null;
        $savedState = $_SESSION['propose_login_state'] ?? null;

        // Clear session data
        unset($_SESSION['propose_login_state'], $_SESSION['propose_data']);

        if (!$code || !$state || !$proposeData || $state !== $savedState) {
            $this->redirectWithError('提案驗證失敗或已過期，請重新填寫表單');
        }
        ...
        // 3. Save Petition and Signature to Database
        try {
            $pdo = Database::getInstance();
            $pdo->beginTransaction();
            ...
            // Insert Petition
            $stmt = $pdo->prepare('INSERT INTO petitions (title, description, category, town, status, target_count, current_count) VALUES (?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $proposeData['title'],
                $proposeData['description'],
                $proposeData['category'],
                $proposeData['town'],
                '審核中',
                50,
                1 // Initially 1 (the creator)
            ]);
            $petitionId = (int)$pdo->lastInsertId();

            // Insert Signature for the creator
            $sigStmt = $pdo->prepare('INSERT IGNORE INTO petition_signatures (petition_id, line_user_id, line_display_name, line_picture_url, town) VALUES (?, ?, ?, ?, ?)');
            $sigStmt->execute([$petitionId, $userId, $displayName, $pictureUrl, $userTown]);

            $pdo->commit();

            $this->redirectWithSuccess('提案已送出！待辦公室審核通過後即會公開連署。');

        } catch (Throwable $e) {
            if ($pdo->inTransaction()) {
                $pdo->rollBack();
            }
            error_log('Propose callback processing error: ' . $e->getMessage());
            $this->redirectWithError('寫入提案資料時發生錯誤');
        }
    }
